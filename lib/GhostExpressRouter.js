...
let _authService, _permissionService, _logger;
...
class GhostExpressRouter {
  ...
  _attachScopedModelServicesToRequest (req, res, next) {
    if (_.isArray(req.creds.permissions)) {
      req.scopes = _.map(req.creds.permissions, 'modelScope');
    }
  ...
  _ensureValid (req, res, next) {
    ...
    else Celebrate(req.routeOptions.validate, { context: { creds: req.creds } })(req, res, (err) => {
      if (!err) next();
      else {
          this._logError('GhostExpressRouter._ensureValid failed', {
              err: err,
              body: req.body,
              params: req.params,
              query: req.query,
        ...
        res.boom.badRequest(err.details[0].message)
      }
    })
  }
  ...
  _logError (errMessage, err) {
    if (_logger) _logger.error(errMessage, err)
  }
