...
let _authService, _db, _permissionService;

class GhostExpressRouter {

  // TODO - ghost-express-server
  // TODO - add 401, 403, 404, & 500 templates to return when those errors occur
  ...
  _attachScopedModelServicesToRequest (req, res, next) {
    if (req.creds && _.isArray(req.creds.permissions)) {
      _.each(req.creds.permissions, permission => {
        ...
        console.log('_attachScopedModelServicesToRequest',modelScope, serviceName, req[serviceName])
      });
    }
  ...
  _ensureAuthenticated (req, res, next) {
    console.log('_ensureAuthenticated')
